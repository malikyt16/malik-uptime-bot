...
client.login("NzU3NDkzNzk2MzU0MjYxMDIy.X2hNEg.Cu8YAXQHfeVyCz4Jvi-35UZV29A");
...
client.on("ready", () => {
    ...
    client.user.setPresence({ activity: { name: `!ekle`, type: "STREAMING", url: "!ekle" } })
...
client.on("message", message => {
  ...
  if(spl[0] == "w!liste") {
  var link = spl[1]
 message.channel.send(`${db.get("linkler").length} Bot`)
}})
